...
class TscanTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        # Load pre-trained weights for finetuning:
        if self.config.TRAIN.PT_PATH:
            if not os.path.exists(self.config.TRAIN.PT_PATH):
                raise ValueError("Pretrained model path error! Please check TRAIN.PT_PATH in your yaml.")
            self.model.load_state_dict(torch.load(self.config.TRAIN.PT_PATH))
            print("Finetuning on top of the pretrained model!")
    ...
    def test(self, data_loader):
        ...
        # Dump the output and label waveforms:
        """
        for sub, chunk_dict in predictions.items():
            for chunk, wave in chunk_dict.items():
                hf = h5py.File(os.path.join('/scratch/manne.sa/data/ACL_23/ACL/output', sub+'_output_'+str(chunk)+'.hdf5'), 'w')
                hf.create_dataset('respiration', data=np.reshape(wave.cpu(), (-1)))
                hf.close()

        for sub, chunk_dict in labels.items():
            for chunk, wave in chunk_dict.items():
                hf = h5py.File(os.path.join('/scratch/manne.sa/data/ACL_23/ACL/output', sub+'_label_'+str(chunk)+'.hdf5'), 'w')
                hf.create_dataset('respiration', data=np.reshape(wave.cpu(), (-1)))
                hf.close()
        """
